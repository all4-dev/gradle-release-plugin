                ...
                if (
                    ...
                    val githubActor =
                        (findProperty("GITHUB_ACTOR") as? String ?: System.getenv("GITHUB_ACTOR"))
                            ?.let { OnePasswordSupport.resolve(it, this) }
                    val githubToken =
                    ...
                    if (customRepo.enabled.orNull == true && customRepo.url.isPresent) {
                        maven {
                            ...
                            if (customRepo.username.isPresent && customRepo.password.isPresent) {
                                val resolvedUsername = OnePasswordSupport.resolve(customRepo.username.get(), this@applyToModule)
                                val resolvedPassword = OnePasswordSupport.resolve(customRepo.password.get(), this@applyToModule)
                                credentials {
                            ...
                            if (
                                ...
                                val resolvedHeaderValue = OnePasswordSupport.resolve(customRepo.authHeaderValue.get(), this@applyToModule)
